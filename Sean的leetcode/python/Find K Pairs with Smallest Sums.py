...
class Solution(object):
    def kSmallestPairs(self, nums1, nums2, k):
        ...
        if not nums1 or not nums2:
            return []
        q = []
        heapq.heappush(q, Value(nums1[0] + nums2[0], 0, 0))
        re = []
        while q and k:
            cur = heapq.heappop(q)
            re.append([nums1[cur.i], nums2[cur.j]])
            if cur.j + 1 < len(nums2):
                heapq.heappush(q, Value(nums1[cur.i] + nums2[cur.j + 1], cur.i, cur.j + 1))
            if cur.i + 1 < len(nums1) and cur.j == 0:
                heapq.heappush(q, Value(nums1[cur.i + 1] + nums2[cur.j], cur.i + 1, cur.j))
